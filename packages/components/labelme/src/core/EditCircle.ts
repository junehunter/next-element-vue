...
import { getTranslateAndScale, isPointInRectangle, isPointInCircleShape, vertexeTransform } from './utils';
...
export default class EditCircle {
	...
	public drawCircle(vertexes: [number, number][]) {
		if (!vertexes?.length) return;
		this.vertexes = vertexes;
		const [[start_x, start_y], [end_x, end_y]] = vertexes;
		const radius = Math.sqrt(Math.pow(end_x - start_x, 2) + Math.pow(end_y - start_y, 2));
		const ctx = this.ctx;
	...
	private canvasMouseMove(e: MouseEvent) {
		...
		if (this.isMoveEditing) {
			...
			const [[center_x, center_y], [edge_x, edge_y]] = _vertexes;
